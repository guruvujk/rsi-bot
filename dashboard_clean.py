...
<div class="grid2">
    ...
    <div class="box">
        ...
        <table>
            ...
            <tbody id="watchlist-body"><table><td colspan="4" class="empty-msg">Waiting for scan...</td></tr></tbody>
        </table>
    </div>
...
function loadPaperPositions() {
    fetch('/api/auto/portfolio')
        ...
        .catch(err => console.error('Error loading paper positions:', err));
}
...
socket.on('state_update', function(d) {
    ...
    var pnlEl = document.getElementById('pnl'); pnlEl.textContent = fmt(d.pnl || 0); colorVal(pnlEl, d.pnl || 0);
    var opEl = document.getElementById('open_pnl'); opEl.textContent = fmt(d.open_pnl || 0); colorVal(opEl, d.open_pnl || 0);
    var retEl = document.getElementById('return_pct'); retEl.textContent = (d.return_pct || 0) + '%'; colorVal(retEl, d.return_pct || 0);
    ...
    for (var sym in wl) {
        var v = wl[sym];
        var rsiColor = v.rsi < 30 ? '#16a34a' : v.rsi > 70 ? '#dc2626' : '#64748b';
        var rsiPct = Math.min(v.rsi, 100);
        var bClass = v.signal === 'BUY' ? 'badge-buy' : v.signal === 'SELL' ? 'badge-sell' : 'badge-hold';
        var isUsd = sym.includes('-USD');
        var priceStr = v.price ? (isUsd ? '$' + Number(v.price).toLocaleString() : fmt(v.price)) : '—';
        whtml += '<tr>' +
    ...
    for (var i = 0; i < trades.length; i++) {
        var t = trades[i];
        var pnlStr = t.pnl != null ? '<span style="color:' + (t.pnl>=0?'#16a34a':'#dc2626') + ';">' + fmt(t.pnl) + '</span>' : '—';
        var bClass = t.type === 'BUY' ? 'badge-buy' : 'badge-sell';
        thtml += '<tr>' +
